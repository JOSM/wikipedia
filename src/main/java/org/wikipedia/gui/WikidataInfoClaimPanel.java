    ...
    void downloadStatementsFor(final String qId) {
        ...
        new Thread(() -> {
            try {
                ...
                final Optional<Collection<WbgetclaimsResult.Claim>> result = ApiQueryClient.query(WikidataActionApiQuery.wbgetentitiesClaims(qId));
                if (result.isPresent()) {
                    if (qIdBeingDownloaded != null && qIdBeingDownloaded.equals(qId)) {
                        synchronized (mainPanel) {
                            mainPanel.removeAll();
                            mainPanel.setLayout(new GridLayout(result.get().size(), 1));
                            result.get().forEach(claim -> {
                    ...
                    final JLabel errorLabel = new JLabel(I18n.tr("Error getting claims for entity!"));
